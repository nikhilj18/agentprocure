...
# ══════════════════════════════════════════════
    ...
    avl_df = load_avl()
    parts_df_avl = load_parts_avl()
    supplier_df_avl = load_suppliers_avl()

    qualified = avl_df[avl_df['qualification_status'] == 'Qualified']
    suspended = avl_df[avl_df['qualification_status'] == 'Suspended']
    part_sup_count = qualified.groupby('part_no')['supplier_id'].count()
    single_source  = part_sup_count[part_sup_count == 1]
    crit_parts     = parts_df_avl[parts_df_avl['component_class']=='Critical']['part_no']
    crit_single    = single_source[single_source.index.isin(crit_parts)]

    col1,col2,col3,col4 = st.columns(4)
    col1.metric("Total AVL Entries", len(avl_df))
    col2.metric("Qualified", len(qualified))
    col3.metric("Suspended", len(suspended), delta=str(len(suspended)) if len(suspended)>0 else None, delta_color="inverse")
    col4.metric("Single-Source", len(single_source), delta=f"{len(crit_single)} critical" if len(crit_single)>0 else None, delta_color="inverse")

    if len(crit_single) > 0:
        st.warning(f"⚠️ {len(crit_single)} Critical parts single-sourced: {', '.join(crit_single.index.tolist()[:5])}")

    st.divider()
    st.subheader("🔍 Current AVL Entries")
    c1,c2,c3 = st.columns(3)
    fc = c1.selectbox("Filter class",  ["All","Commodity","Critical","Custom"])
    fs = c2.selectbox("Filter status", ["All","Qualified","Suspended","Under Review"])
    fq = c3.text_input("Search part", placeholder="e.g. IGBT")
    ddf = avl_df.copy()
    if fc != "All": ddf = ddf[ddf['component_class']==fc]
    if fs != "All": ddf = ddf[ddf['qualification_status']==fs]
    if fq: ddf = ddf[ddf['part_no'].str.contains(fq, case=False)]
    st.dataframe(ddf[['part_no','description','component_class','supplier_name','country','qualification_status','qualified_date']], use_container_width=True, height=300)

    st.divider()
    st.subheader("➕ Add New Supplier Qualification")
    with st.form("add_avl"):
        ca1,ca2 = st.columns(2)
        sp = ca1.selectbox("Component", parts_df_avl['part_no'].tolist())
        cur = qualified[qualified['part_no']==sp]['supplier_name'].tolist()
        if cur: ca1.info(f"Qualified: {', '.join(cur)}")
        else:   ca1.warning("No qualified suppliers!")
        done_ids = avl_df[(avl_df['part_no']==sp)&(avl_df['qualification_status']=='Qualified')]['supplier_id'].tolist()
        avail = supplier_df_avl[~supplier_df_avl['supplier_id'].isin(done_ids)]
        if avail.empty:
            ca2.info("All suppliers already qualified.")
            nsid = None
        else:
            opts = avail.apply(lambda r: f"{r['supplier_name']} ({r['country']})", axis=1).tolist()
            sel  = ca2.selectbox("New Supplier", opts)
            nsid = avail.iloc[opts.index(sel)]['supplier_id']
        qd = ca2.date_input("Qualification Date", value=date.today())
        if st.form_submit_button("✅ Add", type="primary", use_container_width=True) and nsid:
            try:
                run_insert("INSERT INTO approved_vendor_list (part_no,supplier_id,qualification_status,qualified_date) VALUES (%s,%s,%s,%s) ON CONFLICT (part_no,supplier_id) DO UPDATE SET qualification_status='Qualified',qualified_date=%s", (sp,int(nsid),'Qualified',qd,qd))
                st.success(f"✅ Added!")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
    ...
    st.divider()
    st.subheader("✏️ Update or Suspend")
    with st.form("upd_avl"):
        cu1,cu2,cu3 = st.columns(3)
        eopts = avl_df.apply(lambda r: f"{r['part_no']} — {r['supplier_name']}", axis=1).tolist()
        esel  = cu1.selectbox("AVL Entry", eopts)
        eidx  = eopts.index(esel)
        eavl  = avl_df.iloc[eidx]['avl_id']
        ecur  = avl_df.iloc[eidx]['qualification_status']
        sopts = ["Qualified","Suspended","Under Review"]
        nstat = cu2.selectbox("New Status", sopts, index=sopts.index(ecur) if ecur in sopts else 0)
        ndate = cu3.date_input("Date", value=date.today())
        if st.form_submit_button("💾 Update", use_container_width=True):
            try:
                run_insert("UPDATE approved_vendor_list SET qualification_status=%s,qualified_date=%s WHERE avl_id=%s", (nstat,ndate,int(eavl)))
                st.success(f"Updated to {nstat}")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
                st.error(str(e))
    ...
    if len(single_source) > 0:
        ...
        sdf = qualified[qualified['part_no'].isin(single_source.index)][['part_no','component_class','supplier_name','country']].copy()
        sdf['risk'] = sdf['component_class'].map({'Critical':'🔴 High','Custom':'🟡 Medium','Commodity':'🟢 Low'})
